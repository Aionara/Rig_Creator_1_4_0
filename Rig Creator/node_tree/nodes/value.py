...
Nodes = [
    ...
    ("Vector_Acceleration", "Vector Acceleration", data.ListNodeSocket()["vector_acceleration"], False),
    ("Vector_Direction", "Vector Direction", data.ListNodeSocket()["vector_direction"], False),
    ("Vector_Euler", "Vector Euler", data.ListNodeSocket()["vector_euler"], False),
    ("Vector_Translation", "Vector Translation", data.ListNodeSocket()["vector_translation"], False),
    ("Vector_Velocity", "Vector Velocity", data.ListNodeSocket()["vector_velocity"], False),
    ("Vector_xyz", "Vector xyz", data.ListNodeSocket()["vector_xyz"], False),
...
for node in Nodes:
    class_name = f"RGC_Node_{node[0]}"
    bl_idname = f"RGC_Node_{node[0]}"
    bl_label = f"{node[1]}"

    class_operator = type(class_name, (GeneralNodeData, Node), {
        "bl_idname": bl_idname,
        "bl_label": bl_label,
